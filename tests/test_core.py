...
def test_core_bspline_domain(knots, degree, idx, result):
    my_result = core.bspline_domain(knots, degree, idx)
    assert np.linalg.norm(my_result - result) < 1e-10


@pytest.mark.parametrize("knots", [np.arange(5).astype(float)])
@pytest.mark.parametrize("degree", [0, 1])
def test_core_bspline_domain_l_extra(knots, degree):
    idx = 0
...
@pytest.mark.parametrize("knots", [np.arange(5).astype(float)])
@pytest.mark.parametrize("degree", [0, 1])
def test_core_bspline_domain_r_extra(knots, degree):
