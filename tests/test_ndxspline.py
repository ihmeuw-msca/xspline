...
def test_design_mat(spline, is_grid):
    x = np.linspace(0.0, 1.0, 11)
    y = np.linspace(0.0, 1.0, 11)
    mat = spline.design_mat([x, y], is_grid=is_grid)
    if is_grid:
...
@pytest.mark.parametrize('is_grid', [True, False])
def test_design_dmat(spline, is_grid):
    x = np.linspace(0.0, 1.0, 11)
    y = np.linspace(0.0, 1.0, 11)
    dmat = spline.design_dmat([x, y], [1, 1], is_grid=is_grid)
    if is_grid:
...
@pytest.mark.parametrize('is_grid', [True, False])
def test_design_imat(spline, is_grid):
    x = np.linspace(0.0, 1.0, 11)
    y = np.linspace(0.0, 1.0, 11)
    imat = spline.design_imat([x, y], [x, y], [1, 1], is_grid=is_grid)
