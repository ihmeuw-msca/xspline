...
class bspline:
    ...
    def integrateLinFuncL(self, I0, I1, t0, f0, df0):
        I0 = I0.copy()
        I1 = I1.copy()
        I0[I0 > t0] = t0
        I1[I1 > t0] = t0
        #
        integral = f0*(I1 - t0) + 0.5*df0*(I1 - t0)**2\
                 - f0*(I0 - t0) - 0.5*df0*(I0 - t0)**2
        #
        return integral

    def integrateLinFuncR(self, I0, I1, t0, f0, df0):
        I0 = I0.copy()
        I1 = I1.copy()
        I0[I0 < t0] = t0
        I1[I1 < t0] = t0
...
# general function integral design matrix
# =============================================================================
def designIMat(I0, I1, knots, degree, l_linear=False, r_linear=False):
    import numpy as np
    #
    knots = np.sort(np.array(list(set(knots))))
    # check the input
    ...
    bs = bspline(knots[a:b])
    # 
    M = bs.designIMat(degree, 1, I1, start=I0)
    for j in range(M.shape[1]):
        if l_linear:
            lf = bs.splineF(degree, j+1, bs.t[ 0])
            dlf = bs.splineDF(degree, j+1, 1, bs.t[ 0])
            M[:,j] += bs.integrateLinFuncL(I0, I1, bs.t[ 0], lf, dlf)

        if r_linear:
            rf = bs.splineF(degree, j+1, bs.t[-1])
            drf = bs.splineDF(degree, j+1, 1, bs.t[-1])
            M[:,j] += bs.integrateLinFuncR(I0, I1, bs.t[-1], rf, drf)
